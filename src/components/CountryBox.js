...
export default function CountryBox({ country, addClass }) {
  ...
  return (
    <div
      className={`py-3 px-4 d-flex position-relative justify-content-between ${addClass} 
    ${!tabs ? "flex-column" : ""}`}
    >
      {!tabs && (
        <Link
      ...
      >
        <h3
          className={`m-0 py-3 fs-5 text-white hover-1 ${
            !tabs ? "text-end" : "text-start"
          }`}
        >
          {name?.toUpperCase()}
        </h3>
      ...
      <Link
        ...
        onClick={() => clickHandler()}
        className={`${
          !tabs ? "position-absolute" : "ms-5 align-self-center"
        } arrow-right`}
      >
        <FontAwesomeIcon
